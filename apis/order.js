...
function placeOrder(req,res){
		...
		collection.find({uniqueId:uniqueId}).toArray(function(err, docs) {
			if(docs.length != 0){
				...
				collection.update(
			   	    	{ uniqueId:uniqueId } , { $set:{cart:[],totalCost:0},$push:{orders:order}},function(err,response){
			   	    			if(err){
			   	    				console.log(err)

			   	    			}else{
			   	    				//console.log(response)
			   	    				return res.sendJson({code:200,message:'Order Placed'})
			   	    			}
...
function list(req,res){
	let { uniqueId } = req.payload
		...
		collection.find({uniqueId:uniqueId}).toArray(function(err, docs) {
			if(docs.length != 0){
				return res.sendJson({code:200,data:docs[0].orders})
			}
			else{
				return res.sendJson({code:500,message:'User not exist',data:[]},500)
			}
